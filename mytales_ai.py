...
def normalize_gender(g):
    ...
    '여', '여자', 'girl' -> '여자'
    그 외 -> 그대로
    최종적으로 prompt에는 '남자아이', '여자아이', 그 외 '아이'
    """
...
def pick_goal(payload):
    """
    Wix 쪽에서 topic / education_goal / goal 등 필드명이
    일관되지 않을 수 있으므로 우선순위로 하나 고른다.
    """
...
def call_gpt_story(name, age, gender, goal, max_retries=2):
    ...
    반환: dict (이미 JSON 파싱된 형태 기대. 파싱 실패하면 안전한 최소 스켈레톤)
    """
    if not client:
        return None
...
def call_image_generation(image_guide, must_keep, global_visual):
    ...
    # DALL·E 3 응답은 data[0].b64_json 제공하는 SDK가 일반적
...
def generate_story():
    ...
    logger.info(
        f"[generate-story] name={name} age={age} gender_raw={gender_input} gender_norm={gender_norm} "
        f"goal={goal} mock={mock_mode}"
    )
